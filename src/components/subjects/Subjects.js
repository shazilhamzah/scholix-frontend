...
const Subjects = () => {
  ...
  useEffect(() => {
    if (Array.isArray(exams)) {
      const assignmentExams = exams.filter((exam) => exam.examType === "assignment");
  ...
  return (
    <>
      ...
      <div className="p-4 sm:ml-64">
        <div className="upper-row flex justify-between items-center" style={{ marginBottom: 10 }}>
          {subjects.length > 0 ? (
            <h1 className="text-5xl font-extrabold dark:text-white">{subject.name}</h1>
          ) : (
            <h1 className="text-5xl font-extrabold dark:text-white">No subjects found.</h1>
          )}
        ...
        {/* ACCORDION */}
        {subjects.length > 0 ? (
          <div className="my-5" id="accordion-collapse" data-accordion="collapse">
            {[
              ...
              <div key={section.type}>
                <h2 id={`accordion-collapse-heading-${index}`}>
                  ...
                  >
                    ...
                    <svg
                      className={`w-3 h-3 transition-transform ${openSections[index] ? "rotate-180" : ""}`}
                      aria-hidden="true"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 10 6"
                ...
                >
                  <div className="p-3 border border-b-0 border-gray-200 dark:border-gray-700 dark:bg-gray-900">
                    ...
                    <div style={{ display: "flex", justifyContent: "center" }}>
                      <Link to={`/addexam/${section.type}`} state={{ id: subject._id }}>
                        <button
                          style={{
                            height: "30px",
                            width: "30px",
      ...
      </div>:<p className="p-4 sm:ml-64 text-5xl font-extrabold dark:text-white">Set an active semester first</p>}
    </>
  );
};
